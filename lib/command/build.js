...
var js2lua = _interopRequire(require("js2lua"));

var moment = _interopRequire(require("moment"));

var projectPackage = _interopRequire(require("../../package"));

var JaySchema = _interopRequire(require("jayschema"));

var scipmChildSchemaJson = _interopRequire(require("../../schema/scipmchild.0.1.0.schema.json"));
...
var escapeshell = function escapeshell(cmd) {
    return cmd.replace(/(["\s'$`\\])/g, "\\$1");
};

var build = function build(options, logger) {
    ...
    if (!fs.existsSync(scipmPackageJsonMasterFilePath)) {
        ...
        return { code: 1, level: "error", message: message };
    }
    ...
    try {
        for (var _iterator = listScipmPackageNameEnabled[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            var scipmPackageName = _step.value;
            var _iteratorNormalCompletion2 = true;
            var _didIteratorError2 = false;
            var _iteratorError2 = undefined;

            try {
                for (var _iterator2 = allPackageJsonEnabled[scipmPackageName].scipmchild.luaRequire[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                    var luaRequirePackage = _step2.value;

                    if (!luaRequire.hasOwnProperty(luaRequirePackage)) {
                        luaRequire[luaRequirePackage] = [];
                    }
    ...
    _.each(listScipmPackageNameEnabled, function (scipmPackageName) {
        ...
        scitestartup.push("");
        if (allPackageJsonEnabled[scipmPackageName].scipmchild.SciTEStartup._use === "no") {} else if (allPackageJsonEnabled[scipmPackageName].scipmchild.SciTEStartup._use === "dofile") {
            ...
            try {
                for (var _iterator3 = allPackageJsonEnabled[scipmPackageName].scipmchild.SciTEStartup.dofile[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
    ...
    scitestartup.push("-- ----------------------------------------------------------------------------------------------------------------------------");
    scitestartup.push(fs.readFileSync(__dirname + "/../../lua/scipm.core.toolmenu.lua", "utf8"));
