...
import buildCmd from './command/build';
import winston from 'winston';
import shelljs from 'shelljs';
import path from 'path';

var cli = function(cliArgv) {
    ...
    if (opts.nocolor === true) { consoleOptions.colorize = false; }
    if (opts.hasOwnProperty('verbose')) {
        if (opts.verbose.length > 2) { consoleOptions.level = 'silly'; }
        if (opts.verbose.length === 2) { consoleOptions.level = 'debug'; }
        if (opts.verbose.length === 1) { consoleOptions.level = 'verbose'; }
    }
...
`};
    ...
    if (opts._args.length > 1) {
        output = `only one [COMMAND] required`;
        logger.error(output);
        return {code: 1, level: 'error', test: output};
    }
